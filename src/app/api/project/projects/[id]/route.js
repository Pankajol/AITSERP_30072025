...
export async function GET(req, { params }) {
  try {
    ...
    const tasks = await Task.find({ project: params.id, company: decoded.companyId })
      .populate("assignees", "name email")
      .lean();

    const formattedTasks = tasks.map((t) => ({
      ...t,
      projectedStartDate: t.projectedStartDate ? t.projectedStartDate.toISOString() : null,
      projectedEndDate: t.projectedEndDate ? t.projectedEndDate.toISOString() : null,
      progress: t.progress ?? 0,
    }));

    return NextResponse.json({ ...project, tasks: formattedTasks }, { status: 200 });
...
export async function PUT(req, { params }) {
  try {
    ...
    if (body.customer?.value) body.customer = body.customer.value;
    if (body.salesOrder?.value) body.salesOrder = body.salesOrder.value;
    ...
    // 🔔 Notify members except updater
    if (updated.members?.length) {
      const notifications = updated.members
        .filter(m => m._id.toString() !== decoded.id)
        .map(m => ({
          user: decoded.id,
...
export async function DELETE(req, { params }) {
  try {
    ...
    // 🔔 Notify members that project was deleted
    if (deleted.members?.length) {
      const notifications = deleted.members
        .filter(m => m._id.toString() !== decoded.id)
        .map(m => ({
          user: decoded.id,
          company: decoded.companyId,
