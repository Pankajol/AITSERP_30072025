...
export async function POST(req) {
  ...
  try {
    const { adjustmentDate, adjustmentType, reason, remarks, items, refNumber } = await req.json();
    if (!adjustmentDate || !adjustmentType || !items || items.length === 0) {
      return NextResponse.json({ success: false, message: "Required fields missing" }, { status: 400 });
    }

    for (const itemData of items) {
      const { item, variantId, warehouse, quantity, managedBy, batches, selectedBin, unitPrice } = itemData;
      ...
      if (variantId) {
        ...
        if (adjustmentType === "increase") {
          variantInv.quantity += quantity;
        } else {
          variantInv.quantity = Math.max(0, variantInv.quantity - quantity);
        }
      } else {
        if (adjustmentType === "increase") {
      ...
      // Batch handling (if batch-managed)
      if (managedBy === "batch" && batches && batches.length > 0) {
        if (!Array.isArray(inventory.batches)) inventory.batches = [];
        for (const batch of batches) {
          const batchQty = batch.batchQuantity;
          if (!batch.batchNumber || batchQty <= 0) continue;
          const existingBatch = inventory.batches.find(b => b.batchNumber === batch.batchNumber);
          if (existingBatch) {
            if (adjustmentType === "increase") existingBatch.quantity += batchQty;
            else existingBatch.quantity = Math.max(0, existingBatch.quantity - batchQty);
          } else {
      ...
      // Log stock movement – set reference to null to avoid ObjectId cast error
      await StockMovement.create({
        ...
        movementType,
        quantity,
        reference: null, // prevents CastError – adjust schema if you need real reference
