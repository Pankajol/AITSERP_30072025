...
const [invoice] = await SalesInvoice.create([invoiceData], { session });
    ...
    //   if (!isCopied ||  sourceModel === "salesorder") {
    //     const salesOrder = await SalesOrder.findById(invoiceData.sourceId).session(session);
    //     if (salesOrder) {
    //       const totalQty = salesOrder.items.reduce((sum, i) => sum + i.quantity, 0);
    //       const invoicedQty =
    //         (salesOrder.invoicedQty || 0) +
    //         invoiceData.items.reduce((sum, i) => sum + i.quantity, 0);

    //       salesOrder.invoicedQty = invoicedQty;
    //       salesOrder.status = invoicedQty >= totalQty ? "Invoiced" : "Partially Invoiced";
...
/**
 * ✅ GET ALL SALES INVOICES (WITH AUTH)
 */
...
//     // ✅ Extract optional query parameter for filtering
//     const { searchParams } = new URL(req.url);
//     const customerCode = searchParams.get("customerCode");

//     // ✅ Build query based on company and optional customerCode
//     const query = {
//       companyId: user.companyId,
...
//     // ✅ Handle stock only if invoicing directly from SO
//     if (isCopied && sourceModel === "salesorder") {
//       for (const item of invoiceData.items) {
//         const inventoryDoc = await Inventory.findOne({
//           item: new Types.ObjectId(item.item),
...
//         if (!inventoryDoc) {
//           throw new Error(`Inventory record not found for ${item.item}`);
//         }
...
//             );
//             if (batchIndex === -1) {
//               throw new Error(`Batch ${allocated.batchCode} not found for item ${item.item}`);
//             }
...
//               quantity: item.quantity,
//               reference: invoice._id,
//               remarks: "Sales Invoice (Direct from SO)",
//             },
//           ],
//           { session }
//         );
...
//     // ✅ Update SO or Delivery status
//     if (isCopied) {
//       if (sourceModel === "salesorder") {
//         const salesOrder = await SalesOrder.findById(invoiceData.sourceId).session(session);
//         if (salesOrder) {
//           const totalQty = salesOrder.items.reduce((sum, i) => sum + i.quantity, 0);
//           const invoicedQty = (salesOrder.invoicedQty || 0) + invoiceData.items.reduce((sum, i) => sum + i.quantity, 0);
...
//         }
//       } else if (sourceModel === "delivery") {
//         await Delivery.findByIdAndUpdate(invoiceData.sourceId, { status: "Invoiced" }, { session });
//       }
//     }
