...
const DEFAULT_HEADS = [
  ...
  { name: "Bank Account",        type: "Asset",     group: "Bank Account",     balanceType: "Debit",  isSystemAccount: true },
...
export async function POST(req) {
  try {
    ...
    if (nameDuplicate)
      return NextResponse.json({ success: false, message: `"${name.trim()}" naam ka account pehle se exist karta hai` }, { status: 409 });

    // Duplicate code check (if provided)
    ...
    if (trimmedCode) {
      ...
      if (codeDuplicate)
        return NextResponse.json({ success: false, message: `Code "${trimmedCode}" pehle se use ho raha hai` }, { status: 409 });
    }
    ...
    if (err.code === 11000) {
      ...
      const msg = field === "name" ? "Is naam ka account pehle se exist karta hai" :
...
export async function PUT(req, { params }) {
  try {
    ...
    const { id } = params;
    const body = await req.json();
    const { name, type, group, balanceType, parentId, openingBalance, openingBalanceDate, bankDetails, description, code, isActive } = body;

    const existing = await AccountHead.findOne({ _id: id, companyId: user.companyId });
    if (!existing) return NextResponse.json({ success: false, message: "Account not found" }, { status: 404 });

    // Prevent renaming system accounts
    if (existing.isSystemAccount && name && name !== existing.name) {
      return NextResponse.json({ success: false, message: "System accounts cannot be renamed" }, { status: 403 });
    }
    ...
    console.error("PUT /api/accounts/heads error:", err);
    return NextResponse.json({ success: false, message: err.message }, { status: 500 });
  }
}

// DELETE /api/accounts/heads/:id (soft delete)
export async function DELETE(req, { params }) {
  try {
    ...
    const { id } = params;
    const account = await AccountHead.findOne({ _id: id, companyId: user.companyId });
    if (!account) return NextResponse.json({ success: false, message: "Account not found" }, { status: 404 });

    if (account.isSystemAccount)
      return NextResponse.json({ success: false, message: "System accounts cannot be deleted" }, { status: 403 });
    ...
    console.error("DELETE /api/accounts/heads error:", err);
    return NextResponse.json({ success: false, message: err.message }, { status: 500 });
  }
}
