...
export async function POST(req) {
  ...
  try {
    ...
    if (!decoded || !decoded.companyId) throw new Error("Invalid token payload: Missing companyId");
    ...
    async function processItem(item) {
      ...
      if (!inventoryDoc) throw new Error(`No inventory record found for item ${item.item}`);
      ...
      if (item.batches && item.batches.length > 0) {
        for (const allocated of item.batches) {
          ...
          if (batchIndex === -1) throw new Error(`Batch ${allocated.batchCode} not found for item ${item.item}`);
          ...
          if (isCopiedSO) {
            ...
            if (inventoryDoc.batches[batchIndex].quantity < allocated.allocatedQuantity)
              throw new Error(`Insufficient stock in batch ${allocated.batchCode} for item ${item.item}`);
        ...
        if (isCopiedSO) {
          ...
          if (inventoryDoc.quantity < item.quantity)
            throw new Error(`Insufficient stock for item ${item.item} in warehouse ${item.warehouse}`);
      ...
      await StockMovement.create([{
        ...
        remarks: isCopiedSO ? "Delivery from SO copy – committed reduced" : "Normal Delivery – stock reduction",
    ...
    if (isCopiedSO) {
      ...
      for (const soItem of so.items) {
        const deliveredQty = deliveryData.items.find(i => i.item.toString() === soItem.item.toString())?.quantity || 0;

        if (deliveredQty > 0) anyDelivered = true;
        if (deliveredQty < soItem.quantity) allDelivered = false;
      }
