...
async function fetchOutlookMessage({ messageId, userEmail, supportEmail }) {
  ...
  if (!res.ok) throw new Error("Failed to fetch Outlook message");
...
export async function POST(req) {
  try {
    ...
    for (const event of events) {
      ...
      if (event.clientState !== process.env.OUTLOOK_WEBHOOK_SECRET) continue;
    ...
    return new Response("OK"); // Graph will retry on non-200
  }
}
