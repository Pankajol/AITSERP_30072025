...
function CreditMemoForm() {
  ...
  const handleSaveCreditMemo = useCallback(async () => {
    try {
      ...
      // Filter out empty rows from items before validation and submission
      ...
      for (const [idx, item] of validItems.entries()) {
        if (item.managedBy?.toLowerCase() === "batch") {
          ...
          // Also validate individual batch entries for completeness
          const incompleteBatch = ArrayOf(item.batches).some(b => !b.batchNumber || !b.batchQuantity || Number(b.batchQuantity) <= 0);
          if (incompleteBatch) {
      ...
      const itemsForSubmission = validItems.map((it) => ({
        ...it,
        // Ensure values are numbers at the point of submission
        quantity: Number(it.quantity) || 0,
        allowedQuantity: Number(it.allowedQuantity) || 0,
        creditedQuantity: Number(it.creditedQuantity || it.quantity || 0), // Use quantity as credited quantity
        ...
        managedByBatch: it.managedBy?.toLowerCase() === "batch", // Explicit flag for backend
        // Ensure batches are cleaned up for submission (remove 'id' field, filter empty ones)
        batches: ArrayOf(it.batches).filter(
          (b) =>
            b.batchNumber &&
            b.batchNumber.trim() !== "" &&
            Number(b.batchQuantity) > 0
        ).map(({ id, ...rest }) => ({ // Remove temporary 'id' from frontend
            ...rest,
            batchQuantity: Number(rest.batchQuantity) || 0, // Ensure quantity is number
            expiryDate: rest.expiryDate ? new Date(rest.expiryDate).toISOString() : null, // Convert date to ISO string
        })),
      }));

      // Destructure to omit properties not needed in the main payload or handled separately
      const {
        salesInvoiceId,
        attachments: _, // Omit attachments from JSON payload as they are FormData
        items: __, // Omit original items as we use itemsForSubmission
        ...restData
      } = creditMemoData;
      ...
      const payload = {
        ...
        // Ensure summary values are numbers before sending
        ...
        ...(salesInvoiceId ? { salesInvoiceId } : {}), // Include salesInvoiceId if present
      ...
      formData.append("creditMemoData", JSON.stringify(payload));

      if (removedFiles.length > 0) {
        // Assuming your backend expects an array of publicIds or URLs to remove
        formData.append("removedAttachmentIds", JSON.stringify(removedFiles.map(f => f.publicId || f.fileUrl)));
      }
      if (existingFiles.length > 0) {
        // Send existing files data to backend if it needs to keep track (e.g., for Cloudinary)
        formData.append("existingFiles", JSON.stringify(existingFiles));
      }

      attachments.forEach((file) => formData.append("newAttachments", file));

      const url = isEdit ? `/api/credit-note/${editId}` : "/api/credit-note"; // Consistent API endpoints
      const method = isEdit ? "put" : "post";

      console.log("Sending Payload:", payload); // Debug: Check payload before send
      ...
      const response = await axios({
        ...
        headers: {
          Authorization: `Bearer ${token}`,
          // No need to set Content-Type for FormData; browser sets it automatically to multipart/form-data
        },
      });
      ...
      // Sync Sales Invoice if linked
      if (savedCreditMemo.salesInvoiceId) {
        await syncSalesInvoiceForCreditMemo(savedCreditMemo, token);
      }

      toast.success(
        isEdit ? "Credit Memo updated successfully" : "Credit Memo saved successfully"
      );
      router.push("/admin/credit-memo-veiw"); // Redirect to Credit Memo view
    } catch (err) {
    ...
    editId,
    router,
    syncSalesInvoiceForCreditMemo // Add syncSalesInvoiceForCreditMemo to dependency array
  ]);
