...
import {
  ...
  ArrowLeft,
  Clock,
  Truck, // Still used for the header
  Car, // Added for the progress bar
} from "lucide-react";
...
export default function JobCardPage() {
  const [jobCards, setJobCards] = useState([]); // This will be *my* assigned cards
  const [allJobCards, setAllJobCards] = useState([]); // This will be *all* cards for flow logic
  ...
  useEffect(() => {
    ...
    const fetchJobCards = async () => {
      ...
      try {
        ...
        const editable = {};
        // We need to populate editableData and timersRef for ALL cards
        // so we can read the progress of the previous (other operator's) card
        allCards.forEach((c) => {
          editable[c._id] = {
          ...
          // If card is "in progress" AND belongs to me, start its timer
          const isMyCard = myCards.some((mc) => mc._id === c._id);
          if (c.status === "in progress" && isMyCard) {
            startTimer(c._id);
          }
        });

        setAllJobCards(allCards); // Store all cards for logic
        setJobCards(myCards); // Store only my cards for rendering
        setEditableData(editable); // Store data for all cards
      } catch (err) {
  ...
  }, [currentUser]); // Note: `startTimer` is stable and not needed here
  ...
  const updateJobCard = useCallback(async (id, payload) => {
    try {
      ...
      // Update allJobCards state
      setAllJobCards((prev) =>
        prev.map((jc) => (jc._id === id ? updated : jc))
      );

      // Update jobCards state (my cards)
      setJobCards((prev) =>
        prev.map((jc) => (jc._id === id ? updated : jc))
      );
  ...
  const handleSave = async (id, allowedQty) => {
    ...
    if (completedQty > allowedQty) {
      toast.error(
        `Completed Qty (${completedQty}) cannot exceed Allowed Qty (${allowedQty})`
      );
      return;
    }
  ...
  const handleComplete = async (id, allowedQty) => {
    ...
    if (completedQty < allowedQty) {
      toast.warn(
        `Warning: Completing with ${completedQty} / ${allowedQty} items.`
      );
    }
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
            ...
            {jobCards.map((jc) => {
              ...
              // --- NEW CROSS-OPERATOR FLOW LOGIC ---
              let allowedQty = 0;
              let isCardActive = false;
              let isPrevCardStarted = false; // For dimming logic

              // Find this card's index in the *master list*
              const allCardsIndex = allJobCards.findIndex(
                (card) => card._id === id
              );

              // Find the "real" previous card, if it's in the same production order
              const prevCard =
                allCardsIndex > 0 &&
                allJobCards[allCardsIndex - 1].productionOrder?._id ===
                  jc.productionOrder?._id
                  ? allJobCards[allCardsIndex - 1]
                  : null;

              if (!prevCard) {
                // This is the first card in the production order
                allowedQty = qtyToManufacture;
                isCardActive = status !== "completed";
                isPrevCardStarted = true; // Always allow first card
              } else {
                // This card has a predecessor
                // Get the *editable data* for the previous card
                const prevCardData = editableData[prevCard._id] || {};
              ...
              // --- Define progressColor based on percent ---
              const progressColor =
                progressPercent >= 100
                  ? "bg-green-500"
                  : progressPercent > 0
                  ? "bg-yellow-400"
                  : "bg-gray-300"; // Use gray-300 or bg-transparent

              return (
                <div
                  key={id}
                  className={`bg-white rounded-xl shadow-md overflow-hidden transition-opacity ${
                    !isPrevCardStarted // Dim if previous card hasn't started
                  ...
                  <div className="p-4 flex justify-between items-start">
                    <div className="flex-1">
                      <h3 className="text-lg font-bold">
                        {/* We don't have a reliable 'idx' for numbering, so we remove it */}
                        {jc.operation?.name || "N/A"}
                      </h3>
