...
const VOTER_FORM_FIELDS = [
  ...
  { name: "boothId", label: "Booth ID *", type: "text", required: true },
  { name: "supportLevel", label: "Support Level", type: "select", options: SUPPORT_LEVELS.map(s => ({ value: s, label: s })) },
];
...
export default function VotersPage() {
  ...
  const [filters, setFilters] = useState({ supportLevel: "", caste: "", gender: "" });
  ...
  useEffect(() => { fetchVoters(); }, [token, search, filters]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    if (!form.firstName || !form.boothId) {
      return setError("First Name and Booth ID are required.");
    }
    setSaving(true);
    try {
      const payload = { ...form, booth: form.boothId };
      if (editingId) {
  ...
  return (
    <div className="max-w-screen-xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            value={search} onChange={e => setSearch(e.target.value)} placeholder="Search name, Voter ID, phone..."
          />
        </div>
          ...
          {voters.map(voter => (
            <div key={voter._id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all group">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <h3 className="text-base font-bold text-gray-900">{voter.firstName} {voter.lastName}</h3>
      ...
      {/* Voter Modal (same as party modal) */}
      {modalOpen && (
          ...
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh] flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-gray-900">{editingId ? "Edit Voter" : "New Voter"}</h2>
                  <p className="text-xs text-gray-400">Enter voter information</p>
                </div>
              </div>
