// app/(dashboard)/election/surveys/page.js
"use client";
import React, { useEffect, useState, useMemo } from "react";
import axios from "axios";
import {
  FiPlus, FiTrash2, FiSearch, FiClipboard, FiBarChart2, FiEdit2,FiUserCheck,
} from "react-icons/fi";
      ...
      {loading ? (
              ...
              >
                ...
                <div className="flex gap-1.5">
                  {[1, 2].map((j) => (
                    <div
                      key={j}
                      className="h-5 w-16 bg-gray-100 rounded-full"
                    />
                  ))}
                </div>
              </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-400 font-medium">
            {search ? "No matches" : "No surveys yet"}
          </p>
          {!search && (
              ...
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {survey.constituency?.name || "General"}
                  </p>
                </div>
                
                <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <Link
  href={`/election/surveys/${survey._id}/assign`}
...
>
  <FiUserCheck className="text-xs" />
                  ...
                  {/* ✅ Results Button – अब दिखेगा */}
                  <Link
                    href={`/election/surveys/${survey._id}/results`}
                    className="w-7 h-7 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center hover:bg-purple-100"
                    title="View Results"
                  >
                  ...
                  {/* Edit Button */}
                  <Link
                    href={`/election/surveys/${survey._id}/edit`}
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                ...
                {survey.questionGroups?.length > 0 && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-yellow-50 text-yellow-600 border border-yellow-100">
                    {survey.questionGroups.reduce(
                      (total, g) => total + (g.questions?.length || 0),
                      0
                    )}{" "}
                    questions
                  </span>
