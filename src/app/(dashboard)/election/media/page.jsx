...
import { FiPlus, FiTrash2, FiEdit2, FiSearch, FiRadio, FiX } from "react-icons/fi";
...
export default function MediaPage() {
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get("/api/election/media", {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.title || !form.platform) return setError("Title and Platform are required.");
    setSaving(true);
    try {
      if (editingId) {
        await axios.put(`/api/election/media?id=${editingId}`, form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post("/api/election/media", form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      resetForm();
      ...
      setError(e.response?.data?.message || "Error saving campaign");
    } finally { setSaving(false); }
  };

  const handleDelete = async (id) => {
    if (!confirm("Delete this campaign?")) return;
    await axios.delete(`/api/election/media?id=${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  return (
    <div className="max-w-screen-xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-gray-900">Media Campaigns</h1>
      ...
      <div className="relative mb-5 max-w-sm">
        ...
        <input className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white text-sm focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 placeholder:text-gray-300 transition-all"
          value={search} onChange={e => setSearch(e.target.value)} placeholder="Search campaigns..." />
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="w-16 h-16 rounded-2xl bg-indigo-50 flex items-center justify-center mb-4">
            <FiRadio className="text-3xl text-indigo-300" />
          </div>
          <p className="text-gray-400 font-medium">{search ? "No matches" : "No campaigns yet"}</p>
          {!search && <p className="text-sm text-gray-300 mt-1">Click "Add Campaign" to create one</p>}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(item => (
            <div key={item._id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all group">
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10.5px] font-semibold bg-yellow-50 text-yellow-600 border border-yellow-100">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-indigo-600 flex items-center justify-center">
                  <FiRadio className="text-white text-base" />
                </div>
                <div>
                  <h2 className="text-base font-bold text-gray-900">{editingId ? "Edit Campaign" : "New Campaign"}</h2>
                </div>
              </div>
