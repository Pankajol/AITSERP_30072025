...
export default function LeavesPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      /**
       * RULES:
       * 1. Real company = type=company AND no roles
       * 2. Admin / HR / Manager = manager UI
       * 3. Everyone else = Employee
       */
      if (decoded.type === "company" && (!decoded.roles || decoded.roles.length === 0)) {
        setRole("Company");
      } else if (
        decoded.roles?.includes("Admin") ||
        decoded.roles?.includes("HR") ||
        decoded.roles?.includes("Manager")
      ) {
        setRole("Manager");
      } else {
        setRole("Employee");
  ...
  async function loadData() {
    ...
    try {
      ...
      if (isManager) {
        calls.push(axios.get("/api/hr/leaves", { headers }));
        calls.push(axios.get("/api/hr/leaves/analytics", { headers }));
      }

      const [myRes, balRes, teamRes, analyticsRes] =
        await Promise.allSettled(calls);

      if (myRes.status === "fulfilled")
        setMyLeaves(myRes.value.data.data || []);

      if (balRes.status === "fulfilled")
        setBalance(balRes.value.data.data || null);
  ...
  async function handleSaveLeave() {
    ...
    try {
      ...
      if (editingLeave) {
        await axios.put(`/api/hr/leaves/${editingLeave._id}`, form, { headers });
      } else {
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    {(item.status === "Pending" || isManager) && (
                      <button
                        ...
                        className="bg-slate-100 hover:bg-slate-200 px-2 py-1 rounded-md text-[11px]"
                      >
                        Edit
                      </button>
                    )}
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-lg space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="rounded-lg bg-indigo-600 px-5 py-2 text-xs font-medium text-white hover:bg-indigo-700 disabled:opacity-60"
              >
                {submitting
                  ? "Saving..."
                  : editingLeave
...
//       try {
//         const decoded = jwtDecode(token);
//         setRole(decoded.roles?.[0] || "Employee");
//       } catch (e) {
//         console.error(e);
//       }
//     }
...
//       if (editingLeave) {
//         await axios.put(`/api/hr/leaves/${editingLeave._id}`, form, {
//           headers,
//         });
//       } else {
//         await axios.post("/api/hr/leaves", form, { headers });
