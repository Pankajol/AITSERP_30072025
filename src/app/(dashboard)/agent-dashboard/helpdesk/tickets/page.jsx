...
import { FiRefreshCw, FiCpu, FiCheck, FiX, FiSearch, FiLayers, FiList } from "react-icons/fi";
...
export default function AdminHelpdeskTicketsPage() {
  ...
  async function assignAgent(ticketId, agentId) {
    ...
    } catch { toast("error", "Assign error"); }
    finally { setMapFlag(setAiLoadingMap, ticketId, false); }
  }
  ...
  async function updateStatus(ticketId, status) {
    ...
    } catch { toast("error", "Status error"); }
    finally { setMapFlag(setAiLoadingMap, ticketId, false); }
  }
  ...
  async function acceptAiSuggestion(ticketId) {
    ...
    try {
      ...
      await loadTickets();
    } catch { toast("error", "AI assignment failed"); }
    finally { setMapFlag(setAiLoadingMap, ticketId, false); }
  }
  ...
  if (loading) {
    return (
      <div className="h-screen flex flex-col items-center justify-center text-blue-600 bg-slate-50">
        <FiRefreshCw className="text-4xl mb-4 animate-spin" />
        <span className="font-bold tracking-tight text-slate-600">Syncing Tickets...</span>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-slate-900 flex items-center gap-2">
              Helpdesk <span className="text-blue-600 px-2 py-0.5 bg-blue-50 rounded-lg">Console</span>
            </h1>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={loadTickets}
              className="p-2 bg-white border border-slate-200 shadow-sm rounded-xl hover:bg-slate-50 transition-all text-slate-600"
            >
              <FiRefreshCw size={18} />
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-6 space-y-6">
        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-2 items-center">
          <div className="relative w-full md:flex-1">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-transparent border-none rounded-xl text-sm font-semibold focus:ring-0 placeholder:text-slate-400"
            />
          </div>
          ...
          <select
            ...
            className="w-full md:w-48 px-4 py-3 bg-transparent border-none rounded-xl text-sm font-bold text-slate-700 focus:ring-0 cursor-pointer"
          >
        ...
        {msg && (
          <div className={`p-4 rounded-xl flex items-center gap-3 border animate-in slide-in-from-top-2 ${
              msg.type === "error" ? "bg-red-50 text-red-700 border-red-100" : "bg-emerald-50 text-emerald-700 border-emerald-100"
            }`}
          >
            <span className="text-sm font-bold uppercase tracking-tight">{msg.text}</span>
          </div>
        )}
          ...
          <div className="space-y-3">
            {filteredTickets.length > 0 ? (
              filteredTickets.map((t) => {
                ...
                return (
                  <div key={t._id} className="flex flex-col xl:flex-row gap-4">
                    <div className="flex-1 min-w-0" onClick={() => router.push(`/agent-dashboard/helpdesk/tickets/${t._id}`)}>
                      <TicketCard ticket={t} />
                    </div>

                    <div className="xl:w-[400px] flex items-center gap-2 bg-white border border-slate-200 p-2 rounded-xl shadow-sm">
                      ...
                      <select
                        ...
                        className="w-32 bg-slate-50 border-slate-200 rounded-lg text-[11px] font-black uppercase tracking-wider"
                      >
                        <option value="open">Open</option>
                      ...
                      {suggestion && (
                        <button 
                          ...
                          className="p-2 bg-blue-600 text-white rounded-lg hover:shadow-lg hover:shadow-blue-200 transition-all"
                          title={`Assign to suggested: ${suggestion.agentName}`}
                        >
                          <FiCpu size={16} />
                        </button>
                      )}
              ...
              <div className="py-24 text-center bg-white rounded-3xl border border-dashed border-slate-300">
                <FiSearch className="mx-auto text-slate-200 mb-4" size={48} />
                <p className="text-slate-400 font-bold">No tickets match your search criteria.</p>
              </div>
            )}
          </div>
        )}
            ...
            {Object.entries(kanban).map(([status, list]) => (
              <div key={status} className="flex-shrink-0 w-80">
                <div className="flex items-center justify-between mb-4 px-2">
                  <h2 className="text-xs font-black text-slate-500 uppercase tracking-[0.15em] flex items-center gap-2">
                    {status.replace("_", " ")}
                ...
                <div className="space-y-3 min-h-[600px] p-2 bg-slate-100/40 rounded-2xl border border-slate-200/60">
                  {list.map((t) => (
                    <div
                      key={t._id}
                      className="group p-4 bg-white rounded-xl shadow-sm border border-slate-200 cursor-pointer hover:border-blue-400 hover:shadow-md transition-all"
                      onClick={() => router.push(`/agent-dashboard/helpdesk/tickets/${t._id}`)}
                    >
                      <div className="text-[10px] font-black text-blue-600 mb-2 uppercase tracking-widest">
                        #{t._id.slice(-5)}
                      </div>
                      <div className="font-bold text-slate-800 text-sm mb-3 line-clamp-2 leading-snug group-hover:text-blue-700">
                        {t.subject}
                      </div>
                      <div className="flex items-center justify-between">
                         <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center text-[10px] font-black uppercase">
                               {t.customerId?.customerName?.charAt(0) || "?"}
                            </div>
                            <span className="text-[11px] font-bold text-slate-500">{t.customerId?.customerName || "Guest"}</span>
                         </div>
                      </div>
                    </div>
                  ))}
