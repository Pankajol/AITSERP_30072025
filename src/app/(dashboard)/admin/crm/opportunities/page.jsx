...
export default function OpportunityListPage() {
  ...
  const fetchOpportunities = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get("/api/crm/opportunity?limit=500", {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      // ✅ DELETE expects query param ?id=...
      const res = await axios.delete(`/api/crm/opportunity?id=${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.data.success) {
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          {[
            ...
            {
              label: "Win Rate",
              value: `${Math.round((stats.closedWon / stats.total) * 100 || 0)}%`,
              emoji: "📈",
              filter: "All",
            },
          ].map((s) => (
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["#", "Opportunity", "Account", "Expected Value", "Stage", "Prob.", "Actions"].map(
                    (h) => (
                      <th
                        key={h}
                        className="px-4 py-3 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400"
                      >
                        {h}
                      </th>
                    )
  ...
  return (
    <span
      className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
        map[stage] || "bg-gray-100 text-gray-500"
      }`}
    >
      {stage || "Draft"}
    </span>
