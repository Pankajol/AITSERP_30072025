...
export default function TicketDetailPage({ params }) {
  ...
  useEffect(() => {
    ...
    // start polling to pick up inbound replies automatically
  ...
  async function load() {
    ...
    try {
      ...
      // optionally sort messages by timestamp so newest are at bottom (or top as you prefer)
      const t = res.data.ticket;
      if (Array.isArray(t.messages)) {
        t.messages.sort((a, b) => {
          const at = new Date(a.createdAt || a.receivedAt || a._createdAt || 0).getTime();
          const bt = new Date(b.createdAt || b.receivedAt || b._createdAt || 0).getTime();
          return at - bt;
        });
      }
      ...
      <div className="space-y-3">
        {Array.isArray(ticket.messages) && ticket.messages.length > 0 ? (
          ticket.messages.map((m) => {
            ...
            // FIX: prefer createdAt, then receivedAt, then fallback to now
