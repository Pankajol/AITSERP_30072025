    ...
    async function loadData() {
      try {
        ...
        const [statsRes, empRes, attRes] = await Promise.all([
          axios.get("/api/hr/dashboard", { headers }),
          axios.get("/api/hr/employees?limit=5", { headers }),
          axios.get("/api/hr/attendance/today", { headers }),
        ]);

        setStats(statsRes.data?.data || statsRes.data || null);
        setLatestEmployees(empRes.data?.data || []);
              ...
              <table className="w-full text-sm">
                <thead className="border-b bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-medium">Punch Out</th>
                ...
                <tbody>
                  ...
                  {todayAttendance.map((row) => (
                    <tr key={row._id} className="border-b last:border-0">
                      ...
                      <td className="px-4 py-3">{row?.totalHours ?? "0"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
