...
// (defining components inside a render function causes focus loss after every keystroke)
const Lbl = ({ text }) => (
...
const fi =
  "w-full px-3 py-2 rounded-lg border border-gray-200 bg-gray-50 text-sm focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all outline-none";
...
export default function InventoryAdjustmentsView() {
  ...
  const filteredAdjustments = useMemo(() => {
    return adjustments
      ...
      .filter((adj) => {
        const matchItem =
          !filters.item ||
          adj?.item?.itemName?.toLowerCase().includes(filters.item.toLowerCase());
        const matchWH =
          !filters.warehouse ||
          adj?.warehouse?.warehouseName?.toLowerCase().includes(filters.warehouse.toLowerCase());
  ...
  const clearFilters = () =>
    setFilters({ item: "", warehouse: "", type: "", remarks: "", quantity: "", date: "" });

  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-gray-50">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-indigo-600 mb-4" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFilter("remarks", e.target.value)}
                placeholder="Keyword..."
              />
            </div>
            <div>
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["Item & Code", "Warehouse", "Movement", "Quantity", "Remarks", "Logged Date"].map((h) => (
                    <th
                      key={h}
                      className="px-6 py-4 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
                ...
                {filteredAdjustments.length > 0 ? (
                  filteredAdjustments.map((adj) => (
                    <tr key={adj._id} className="hover:bg-indigo-50/20 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2 py-1 rounded-full text-[10px] font-black tracking-widest border ${
                            adj.movementType === "IN"
