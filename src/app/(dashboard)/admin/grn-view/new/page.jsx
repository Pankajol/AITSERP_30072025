...
// ============================================================
// HELPERS (same as in PurchaseOrder form)
// ============================================================
...
function GRNForm() {
  ...
  const updateItemValues = useCallback((item) => {
  ...
  useEffect(() => {
    const totalBeforeDiscount = grnData.items.reduce(
      (sum, it) => sum + (it.priceAfterDiscount || 0) * (it.quantity || 0),
      0
    );
    const gstTotal = grnData.items.reduce((sum, it) => sum + (it.gstAmount || 0), 0);
    const freight = Number(grnData.freight) || 0;
    const rounding = Number(grnData.rounding) || 0;
  ...
  // Load data from Purchase Order or existing GRN
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) return;

    if (!isEdit) {
      const storedPO = sessionStorage.getItem("grnData");
      if (storedPO) {
        ...
        setGrnData((prev) => ({
          ...
          items: po.items.map((poItem) => {
            const remainingQty = (Number(poItem.quantity) || 0) - (Number(poItem.receivedQuantity) || 0);
            const baseItem = {
              ...initialGRNState.items[0],
              ...poItem,
              quantity: remainingQty,
              allowedQuantity: remainingQty,
              receivedQuantity: 0,
            };
            return updateItemValues(baseItem);
          }),
        }));
      ...
      setLoading(true);
      axios
        .get(`/api/grn/${editId}`, { headers: { Authorization: `Bearer ${token}` } })
        .then((res) => {
          const rec = res.data.data;
          // ✅ FIX: enhance items with imageUrl from variant if missing
          const enhancedItems = rec.items.map((it) => {
            let imageUrl = it.imageUrl || it.item?.imageUrl || '';
            // If variant object exists and has image, use that
            if (it.variant) {
              if (it.variant.variantImageUrl) imageUrl = it.variant.variantImageUrl;
              else if (it.variant.imageUrl) imageUrl = it.variant.imageUrl;
            }
            return {
              ...initialGRNState.items[0],
              ...it,
              imageUrl,
              allowedQuantity: it.quantity,
            };
          });
          setGrnData({
            ...rec,
        ...
        .catch((err) => toast.error("Failed to load GRN"))
        .finally(() => setLoading(false));
    }
  }, [isEdit, editId, updateItemValues]);
  ...
  const handleItemChange = (index, update) => {
    setGrnData((prev) => {
      ...
      updatedItem = updateItemValues(updatedItem);
  ...
  const addItemRow = () => {
    const defaultWarehouse = warehouses.find((w) => w._id === selectedGlobalWarehouse);
    setGrnData((prev) => ({
      ...prev,
      items: [
        ...prev.items,
  ...
  const handleSubmit = async () => {
    ...
    if (grnData.items.some((it) => it.quantity <= 0)) {
      toast.error("All items must have a received quantity > 0.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...grnData,
        postingDate: grnData.postingDate,
        documentDate: grnData.documentDate,
        existingFiles,
        removedFiles,
      ...
      fd.append("grnData", JSON.stringify(payload));
  ...
  if (loading && !grnData.supplierName) {
    return <div className="p-10 text-center text-gray-400">Loading GRN data...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        >
          <FaArrowLeft className="text-xs" /> Back to GRN List
        </button>

        <h1 className="text-2xl font-extrabold text-gray-900 mb-6">
          {isEdit ? "Edit Goods Receipt Note" : "New Goods Receipt Note (GRN)"}
        </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-1">
              {grnData.supplierName ? (
                <ReadField label="Supplier Name" value={grnData.supplierName} />
              ) : (
                <SupplierSearch
                  onSelectSupplier={(s) =>
                    setGrnData((prev) => ({
                      ...
                      contactPerson: s.contactPersonName || s.contactPerson,
                    }))
                  }
                />
              )}
        ...
        {/* Dates & Status */}
        <SectionCard icon={FaCalendarAlt} title="Timeline & Status" color="blue">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <Lbl text="Posting Date" req />
              <DatePicker
                selected={new Date(grnData.postingDate)}
                onChange={(date) => setGrnData((prev) => ({ ...prev, postingDate: formatDateForInput(date) }))}
                className={fi()}
            ...
            <div>
              ...
              <DatePicker
                selected={grnData.documentDate ? new Date(grnData.documentDate) : null}
                onChange={(date) => setGrnData((prev) => ({ ...prev, documentDate: formatDateForInput(date) }))}
                className={fi()}
                dateFormat="dd-MM-yyyy"
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
          ...
          <div className="px-6 py-5">
            <div className="flex flex-wrap items-center gap-3">
              <select
                className="px-3 py-2 rounded-lg border border-gray-200 text-sm bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100"
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <FaCopy className="text-xs" /> Apply to All Items
              </button>
            </div>
            <p className="text-xs text-gray-400 mt-3">
              This will overwrite the warehouse for <strong>all existing items</strong>.
            </p>
          </div>
        </div>

        {/* Items Section */}
        <div className="bg-white rounded-2xl shadow-sm border mb-5 overflow-visible">
          <div className="px-6 py-4 border-b bg-emerald-50/40 font-bold flex items-center gap-2">
            <FaBoxOpen className="text-emerald-500" /> Items Received
          </div>
        ...
        <div className="flex items-center justify-between pt-4 pb-10">
          <button
            onClick={() => router.push("/admin/grn-view")}
            className="px-6 py-2.5 rounded-xl bg-white border border-gray-200 font-bold text-sm hover:bg-gray-50 transition-all"
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className={`px-8 py-2.5 rounded-xl text-white font-bold text-sm shadow-lg transition-all ${
