...
export default function GRNList() {
  ...
  // ✅ Stats state (global, not affected by pagination)
  ...
  // ✅ Fetch global stats (once, without pagination)
  const fetchStats = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      if (!token) return;
      const res = await axios.get("/api/grn?limit=10000", {
      ...
      if (res.data?.success) {
        ...
        const completed = all.filter(g => g.status === "Completed" || g.status === "Closed").length;
        const totalValue = all.reduce((sum, g) => sum + (Number(g.grandTotal) || 0), 0);
        setStats({
          total: all.length,
          open,
          pending,
  ...
  const fetchGRNs = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/grn?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (res.data?.success) {
        ...
        toast.error(res.data?.error || "Failed to fetch GRNs");
      }
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      fetchGRNs(); // refresh current page
  ...
  // ✅ FIX: Copy to Invoice – fetch full GRN with variants first
  const handleCopyToInvoice = async (grn) => {
    if (!grn || !grn._id) {
    ...
    try {
      const token = localStorage.getItem("token");
      // Fetch complete GRN with populated item.variants
      const res = await axios.get(`/api/grn?id=${grn._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.data.success) {
        const fullGrn = res.data.data;
        const dataToStore = {
          ...fullGrn,
          invoiceType: "GRNCopy",
          items: fullGrn.items || [],
        ...
        toast.error("Failed to load full GRN details");
      }
    } catch (error) {
      console.error("Copy error:", error);
      toast.error("Could not copy GRN. Please try again.");
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
          ...
          <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-gray-100 bg-white">
            ...
            <div className="flex gap-2 flex-wrap ml-auto">
              {["All", "Open", "Pending", "Completed", "Received", "Partial"].map(s => (
              ...
              <div key={g._id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  <span className="font-mono text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">
                    {g.documentNumberGrn || `#${(page-1)*limit + idx + 1}`}
                  </span>
                  <RowMenu grn={g} onDelete={handleDelete} onCopy={handleCopyToInvoice} onPrint={handlePrint} />
                </div>
