...
import CRMAgentDashboard from "./components/CRMAgentDashboard";
...
export default function DashboardRouter() {
  ...
  const isCRM = roles.includes("CRM Agent") || roles.includes("CRM");
  const isHR = roles.includes("HR") || roles.includes("HR Agent");
  const isAgent = roles.includes("Agent") && !isAdmin && !isCompany;

  if (isAdmin || isCompany) return <AdminDashboard session={session} />;
  if (isCRM) return <CRMAgentDashboard session={session} />;
  if (isHR) return <HRDashboard session={session} />;
  if (isAgent) return <AgentDashboard session={session} />;

  // Fallback
  return <DefaultDashboard session={session} />;
}

// Simple fallback dashboard
function DefaultDashboard({ session }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-8">
      <div className="max-w-2xl mx-auto text-center mt-20">
        <h1 className="text-2xl font-bold text-gray-800">Welcome, {session.name}</h1>
        <p className="text-gray-500 mt-2">Your role does not have a custom dashboard yet.</p>
        <p className="text-sm text-gray-400 mt-4">Please contact your administrator.</p>
      </div>
    </div>
  );
}
