...
import {
  ...
  FaPaperclip, FaTimes, FaFileInvoice, FaMoneyBillWave, FaCopy, FaWarehouse
} from "react-icons/fa";
...
const initialState = {
  ...
  items: [{
    ...
    priceAfterDiscount: 0, totalAmount: 0, gstAmount: 0, cgstAmount: 0, sgstAmount: 0,
    igstRate: 0, igstAmount: 0, tdsAmount: 0,
    warehouse: "", warehouseName: "", warehouseCode: "",
    stockAdded: false, managedBy: "", batches: [],
...
// Helper: get variant image URL from populated item
...
function PurchaseInvoiceForm() {
  ...
  // Fetch warehouses and bank accounts
  useEffect(() => {
  ...
  const applyWarehouseToAll = useCallback(() => {
    if (!selectedGlobalWarehouse) {
      toast.warning("Please select a warehouse first.");
      return;
    }
    const wh = warehouses.find(w => w._id === selectedGlobalWarehouse);
    if (!wh) return;
    setInvoiceData(prev => ({
      ...prev,
  ...
  }, [selectedGlobalWarehouse, warehouses]);
  ...
  // Load source (GRN or previous invoice) or edit
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (isEdit && editId) {
      setLoading(true);
      // ✅ FIX API endpoint: use query parameter instead of path param
      axios.get(`/api/purchaseInvoice?id=${editId}`, { headers: { Authorization: `Bearer ${token}` } })
        .then(res => {
      ...
      // Try to load from session storage (GRN or previous invoice copy)
      ...
      const sourceData = grnData || piData;
      if (sourceData) {
        try {
          let src = JSON.parse(sourceData);
          
          // ✅ SAFETY FALLBACK: If this is a copy from a Purchase Order but required fields are missing, set them
          if (src.purchaseOrderId && !src.purchaseOrder) {
            src.purchaseOrder = src.purchaseOrderId;
          }
          if (src.sourceType === "PI" && src.sourceId && !src.invoiceType && !src.purchaseOrder) {
            // Attempt to derive from sourceId – but better to let user fix PO copy function
          }
          if (src.purchaseOrder && (!src.invoiceType || src.invoiceType !== "POCopy")) {
            src.invoiceType = "POCopy";
            console.log("✅ Fallback: Set invoiceType = POCopy because purchaseOrder is present");
          }
          
          // Build items with proper imageUrl from variants
          ...
          setInvoiceData(prev => ({
            ...prev,
            ...src, // includes invoiceType and purchaseOrder if present
            supplier: src.supplier?._id || src.supplier,
  ...
  const [paymentFields, setPaymentFields] = useState({
    ...
    cardLast4Digits: "", cardNetwork: "", chequeNumber: "", chequeDate: "",
    bankName: "", paymentDate: formatDateForInput(new Date()), notes: ""
  });
  ...
  const addPayment = () => {
    ...
    if (paymentMethod === "card") {
      newPayment.cardLast4Digits = paymentFields.cardLast4Digits;
      newPayment.cardNetwork = paymentFields.cardNetwork;
      newPayment.transactionId = paymentFields.transactionId;
    }
    if (paymentMethod === "netbanking" || paymentMethod === "wallet") {
      newPayment.transactionId = paymentFields.transactionId;
      newPayment.paymentGateway = paymentFields.paymentGateway;
    }
    if (paymentMethod === "cheque") {
      newPayment.chequeNumber = paymentFields.chequeNumber;
      newPayment.chequeDate = paymentFields.chequeDate;
      newPayment.bankName = paymentFields.bankName;
    }
    setInvoiceData(prev => ({ ...prev, payments: [...(prev.payments || []), newPayment] }));
    setPaymentAmount(0);
    setPaymentFields({
      bankAccountId: "", upiId: "", transactionId: "", paymentGateway: "",
      cardLast4Digits: "", cardNetwork: "", chequeNumber: "", chequeDate: "",
  ...
  const handleSubmit = async () => {
    ...
    if (invoiceData.items.some(it => it.quantity <= 0)) {
      toast.error("All items must have a valid quantity > 0.");
      return;
    }
    setLoading(true);
    try {
      ...
      const url = isEdit ? `/api/purchaseInvoice?id=${editId}` : "/api/purchaseInvoice";
  ...
  if (loading && isEdit && !invoiceData.supplierName) {
    return <div className="p-10 text-center text-gray-400">Loading invoice data...</div>;
  }
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        {invoiceData.sourceId && (
          <div className="mb-5 bg-indigo-50 border-l-4 border-indigo-400 p-4 rounded-r-xl flex items-center gap-3">
            ...
            <p className="text-sm text-indigo-700 font-bold">Source: {invoiceData.sourceType}</p>
          </div>
        )}
        ...
        <SectionCard icon={FaUser} title="Supplier Details" color="indigo">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-1">
              <SupplierSearch
                onSelectSupplier={s => setInvoiceData(p => ({ ...p, supplier: s._id, supplierCode: s.supplierCode, supplierName: s.supplierName, contactPerson: s.contactPersonName }))}
                initialSupplier={invoiceData.supplier ? { _id: invoiceData.supplier, supplierName: invoiceData.supplierName } : undefined}
              />
            </div>
            <ReadField label="Supplier Code" value={invoiceData.supplierCode} />
            <ReadField label="Contact Person" value={invoiceData.contactPerson} />
            <div>
              <Lbl text="Invoice Reference No." />
              <input className={fi()} name="refNumber" value={invoiceData.refNumber || ""} onChange={handleInputChange} placeholder="INV-001" />
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard icon={FaCalendarAlt} title="Dates & Status" color="blue">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <DatePicker
                selected={invoiceData.postingDate ? new Date(invoiceData.postingDate) : null}
                onChange={date => handleInputChange({ target: { name: "postingDate", value: formatDateForInput(date) } })}
                className={fi()}
            ...
            <div>
              ...
              <DatePicker
                selected={invoiceData.documentDate ? new Date(invoiceData.documentDate) : null}
                onChange={date => handleInputChange({ target: { name: "documentDate", value: formatDateForInput(date) } })}
                className={fi()}
                dateFormat="dd-MM-yyyy"
              />
            </div>
            <div>
              ...
              <DatePicker
                ...
                onChange={date => handleInputChange({ target: { name: "dueDate", value: formatDateForInput(date) } })}
                className={fi()}
                dateFormat="dd-MM-yyyy"
              />
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
            ...
            <div className="px-6 py-5">
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button onClick={applyWarehouseToAll} disabled={!selectedGlobalWarehouse}
        ...
        <div className="bg-white rounded-2xl shadow-sm border mb-5 overflow-hidden">
          <div className="px-6 py-4 border-b bg-emerald-50/40 font-bold flex items-center gap-2">
            <FaBoxOpen className="text-emerald-500" /> Invoice Items
          </div>
        ...
        <SectionCard icon={FaCalculator} title="Financial Summary" color="amber">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <ReadField label="Taxable Amount" value={`₹ ${invoiceData.totalBeforeDiscount.toFixed(2)}`} />
            ...
            {invoiceData.payments.length > 0 && (
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead><tr className="border-b"><th>Amount</th><th>Method</th><th>Details</th><th>Date</th><th></th></tr></thead>
                  <tbody>
                    {invoiceData.payments.map((p, idx) => (
                      <tr key={idx} className="border-b border-gray-100">
                        ...
                        <td className="text-xs text-gray-500">
                          {p.method === "bank" && bankAccounts.find(b => b._id === p.bankAccountId)?.name}
                          {p.method === "upi" && p.upiId}
                          {p.method === "card" && `${p.cardNetwork} ****${p.cardLast4Digits}`}
                          {p.method === "cheque" && `Cheque ${p.chequeNumber}`}
                          {p.transactionId && ` | TX: ${p.transactionId}`}
                        </td>
                        <td>{p.paymentDate ? new Date(p.paymentDate).toLocaleDateString() : "—"}</td>
                        <td><button onClick={() => removePayment(idx)} className="text-red-500 text-xs">Remove</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                <div className="mt-3 p-3 bg-gray-100 rounded-lg flex justify-between">
                  <span className="font-bold">Total Paid:</span><span className="font-mono font-bold text-green-700">₹{totalPaid.toLocaleString()}</span>
                  <span className="font-bold">Remaining:</span><span className="font-mono font-bold text-red-600">₹{remaining.toLocaleString()}</span>
                </div>
              </div>
            )}
          </div>
...
//   FaBoxOpen, FaCalculator, FaPaperclip, FaTimes, FaFileInvoice, FaMoneyBillWave
// } from "react-icons/fa";
...
//       try {
//         const token = localStorage.getItem("token");
//         const res = await axios.get("/api/accounts/heads?type=Asset&group=Bank Account", {
...
//       newPayment.upiId = paymentFields.upiId;
//       newPayment.transactionId = paymentFields.transactionId;
//     }
//     if (paymentMethod === "card") {
//       newPayment.cardLast4Digits = paymentFields.cardLast4Digits;
...
//   const totalPaid = purchaseInvoiceData.payments.reduce((sum, p) => sum + (p.amount || 0), 0);
//   const remaining = purchaseInvoiceData.grandTotal - totalPaid;
...
//                 <div>
//                   <Lbl text="Amount" />
//                   <input type="number" className={fi()} value={paymentAmount} onChange={e => setPaymentAmount(Number(e.target.value))} placeholder="0.00" />
//                 </div>
//                 <div>
//                   <Lbl text="Method" />
...
//                     <Lbl text="Bank Account" />
//                     <select className={fi()} value={paymentFields.bankAccountId} onChange={e => setPaymentFields({...paymentFields, bankAccountId: e.target.value})}>
...
//                 {paymentMethod === "upi" && (
//                   <>
//                     <div><Lbl text="UPI ID" /><input className={fi()} value={paymentFields.upiId} onChange={e => setPaymentFields({...paymentFields, upiId: e.target.value})} placeholder="vendor@bank" /></div>
//                     <div><Lbl text="Transaction ID" /><input className={fi()} value={paymentFields.transactionId} onChange={e => setPaymentFields({...paymentFields, transactionId: e.target.value})} placeholder="TX123456" /></div>
//                   </>
//                 )}
//                 {paymentMethod === "card" && (
//                   <>
//                     <div><Lbl text="Last 4 digits" /><input className={fi()} maxLength="4" value={paymentFields.cardLast4Digits} onChange={e => setPaymentFields({...paymentFields, cardLast4Digits: e.target.value})} placeholder="1234" /></div>
//                     <div><Lbl text="Network" /><select className={fi()} value={paymentFields.cardNetwork} onChange={e => setPaymentFields({...paymentFields, cardNetwork: e.target.value})}><option value="">Select</option><option>Visa</option><option>Mastercard</option><option>Amex</option><option>RuPay</option></select></div>
//                     <div><Lbl text="Transaction ID" /><input className={fi()} value={paymentFields.transactionId} onChange={e => setPaymentFields({...paymentFields, transactionId: e.target.value})} /></div>
//                   </>
//                 )}
//                 {(paymentMethod === "netbanking" || paymentMethod === "wallet") && (
//                   <>
//                     <div><Lbl text="Transaction ID" /><input className={fi()} value={paymentFields.transactionId} onChange={e => setPaymentFields({...paymentFields, transactionId: e.target.value})} /></div>
//                     <div><Lbl text="Gateway" /><input className={fi()} value={paymentFields.paymentGateway} onChange={e => setPaymentFields({...paymentFields, paymentGateway: e.target.value})} placeholder="Razorpay / PayU" /></div>
//                   </>
//                 )}
//                 {paymentMethod === "cheque" && (
//                   <>
//                     <div><Lbl text="Cheque No." /><input className={fi()} value={paymentFields.chequeNumber} onChange={e => setPaymentFields({...paymentFields, chequeNumber: e.target.value})} /></div>
//                     <div><Lbl text="Cheque Date" /><input type="date" className={fi()} value={paymentFields.chequeDate} onChange={e => setPaymentFields({...paymentFields, chequeDate: e.target.value})} /></div>
//                     <div><Lbl text="Bank Name" /><input className={fi()} value={paymentFields.bankName} onChange={e => setPaymentFields({...paymentFields, bankName: e.target.value})} /></div>
//                   </>
//                 )}
...
//                 <div className="sm:col-span-3"><Lbl text="Notes" /><input className={fi()} value={paymentFields.notes} onChange={e => setPaymentFields({...paymentFields, notes: e.target.value})} placeholder="Optional" /></div>
//                 <div className="flex items-end"><button type="button" onClick={addPayment} className="bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">Add Payment</button></div>
//               </div>
//             </div>
...
//               <div className="overflow-x-auto">
//                 <table className="min-w-full text-sm">
//                   <thead><tr className="border-b"><th className="text-left py-2">Amount</th><th className="text-left">Method</th><th className="text-left">Details</th><th className="text-left">Date</th><th></th></tr></thead>
//                   <tbody>
//                     {purchaseInvoiceData.payments.map((pmt, idx) => (
//                       <tr key={idx} className="border-b border-gray-100">
//                         <td className="py-2 font-mono">₹{pmt.amount.toLocaleString()}</td>
//                         <td className="capitalize">{pmt.method}</td>
//                         <td className="text-xs text-gray-500">
//                           {pmt.method === "bank" && pmt.bankAccountId && bankAccounts.find(b => b._id === pmt.bankAccountId)?.name}
//                           {pmt.method === "upi" && pmt.upiId}
//                           {pmt.method === "card" && `${pmt.cardNetwork} ****${pmt.cardLast4Digits}`}
//                           {pmt.method === "cheque" && `Cheque ${pmt.chequeNumber}`}
//                           {pmt.transactionId && ` | TX: ${pmt.transactionId}`}
//                         </td>
//                         <td>{pmt.paymentDate ? new Date(pmt.paymentDate).toLocaleDateString() : "—"}</td>
//                         <td><button onClick={() => removePayment(idx)} className="text-red-500 text-xs">Remove</button></td>
//                       </tr>
//                     ))}
//                   </tbody>
...
//                   <span className="font-mono font-bold text-green-700">₹{totalPaid.toLocaleString()}</span>
//                   <span className="font-bold">Remaining:</span>
//                   <span className="font-mono font-bold text-red-600">₹{remaining.toLocaleString()}</span>
//                 </div>
//               </div>
//             )}
//           </div>
