...
const SectionCard = ({ icon: Icon, title, subtitle, children, color = "indigo" }) => {
  // Static color classes to avoid Tailwind dynamic issues
  const colorMap = {
...
// Helper to get variant image from populated item
...
function DebitNoteForm() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/warehouse", { headers: { Authorization: `Bearer ${token}` } })
      .then(res => { if (res.data.success) setWarehouses(res.data.data); })
      .catch(console.error);
  }, []);
  ...
  // Recalculate totals
  useEffect(() => {
    const totalBefore = formData.items.reduce((sum, it) => sum + (it.priceAfterDiscount || 0) * (it.quantity || 0), 0);
  ...
  // Load data (edit or copy from invoice)
  useEffect(() => {
    const token = localStorage.getItem("token");
    const loadData = async () => {
      if (isEdit && editId) {
        setLoading(true);
        try {
          // ✅ Use query parameter (like other modules)
          const res = await axios.get(`/api/debit-note?id=${editId}`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          const rec = res.data?.data || res.data;
          if (rec) {
            // Enhance items with imageUrl from variants
            const enhancedItems = (rec.items || []).map(it => {
        ...
        // Copy from invoice (Purchase Invoice)
        ...
        if (stored) {
          try {
            ...
            const mappedItems = (source.items || []).map(it => {
              let imageUrl = it.imageUrl || it.item?.imageUrl || '';
              if (it.item && it.itemCode && it.itemCode !== it.item?.itemCode && it.item.variants) {
                const variantImg = getVariantImageUrl(it.item, it.itemCode);
                if (variantImg) imageUrl = variantImg;
            ...
            setFormData(prev => ({
              ...
              sourceId: source._id,
              sourceType: source.invoiceType === "GRNCopy" ? "GRN" : "Invoice",
              items: mappedItems,
            }));
  ...
  const openBatchModal = async (index) => {
    const item = formData.items[index];
    if (!item.item || !item.warehouse) {
      toast.warn("Select Item and Warehouse first.");
      return;
    }
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`/api/inventory-batch/${item.item}/${item.warehouse}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.data.success) {
  ...
  const handleSubmit = async () => {
    ...
    if (formData.items.some(it => it.quantity <= 0)) {
    ...
    try {
      ...
      fd.append("debitNoteData", JSON.stringify(payload));
      attachments.forEach(file => fd.append("attachments", file));

      const url = isEdit ? `/api/debit-note?id=${editId}` : "/api/debit-note";
      ...
      console.error(err);
      toast.error(err.response?.data?.error || "Error saving Debit Note");
    } finally {
      setLoading(false);
    }
  };

  if (loading && isEdit && !formData.supplierName) {
    return <div className="p-10 text-center text-gray-400">Loading Debit Note...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        {formData.sourceId && (
          <div className="mb-5 bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-xl flex items-center gap-3">
            <FaCopy className="text-orange-500" />
            <p className="text-sm text-orange-700 font-bold">Copied from {formData.sourceType}</p>
          </div>
        )}
        ...
        <SectionCard icon={FaUser} title="Supplier Details" color="orange">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-1">
              {formData.supplierName && (isEdit || formData.sourceId) ? (
                ...
                <>
                  ...
                  <SupplierSearch
                    onSelectSupplier={s => setFormData(p => ({
            ...
            <div>
              <Lbl text="Debit Note Reference" />
              <input className={fi()} name="refNumber" value={formData.refNumber || ""} onChange={handleInputChange} placeholder="e.g. DN-2024-001" />
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard icon={FaCalendarAlt} title="Date & Status" color="blue">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" className={fi()} name="postingDate" value={formData.postingDate} onChange={handleInputChange} />
            </div>
            <div>
              <Lbl text="Document Date" />
              <input type="date" className={fi()} name="documentDate" value={formData.documentDate} onChange={handleInputChange} />
            </div>
            <div>
              <Lbl text="Status" />
        ...
        {warehouses.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-5">
            ...
            <div className="px-6 py-5">
              ...
              <p className="text-xs text-gray-400 mt-3">This will overwrite the warehouse for <strong>all existing items</strong>.</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border mb-5 overflow-hidden">
          <div className="px-6 py-4 border-b bg-emerald-50/40 font-bold flex items-center gap-2">
            <FaBoxOpen className="text-emerald-500" /> Items Information
          </div>
        ...
        {/* Batch allocation buttons (if items are batch-managed) */}
        {formData.items.some(it => it.managedBy === "batch") && (
          <div className="mb-5 space-y-2">
            {formData.items.map((item, idx) => item.managedBy === "batch" && (
              <div key={idx} className="p-4 bg-white border rounded-xl flex items-center justify-between shadow-sm">
                <p className="text-sm font-bold">{item.itemName || item.itemCode} <span className="text-xs text-gray-400 ml-2">Qty: {item.quantity}</span></p>
                <button onClick={() => openBatchModal(idx)} className="px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold hover:bg-indigo-100 transition-all">
                  Allocate Batches
                </button>
              </div>
            ))}
        ...
        <SectionCard icon={FaCalculator} title="Financial Summary" color="amber">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className="px-3 py-2.5 rounded-lg border-2 border-orange-200 bg-orange-50 font-extrabold text-orange-700">
                ₹ {formData.grandTotal.toFixed(2)}
              </div>
            </div>
          </div>
          <div className="mt-4">
            <Lbl text="Remarks / Reason" />
            <textarea className={`${fi()} resize-none`} name="remarks" rows={2} value={formData.remarks || ""} onChange={handleInputChange} placeholder="Reason for debit note..." />
          </div>
        </SectionCard>
        ...
        <SectionCard icon={FaPaperclip} title="Attachments" color="gray">
          ...
          <label className="flex items-center justify-center gap-3 px-4 py-4 rounded-xl border-2 border-dashed border-gray-200 cursor-pointer hover:bg-orange-50 transition-all group">
            <FaPaperclip className="text-gray-300 group-hover:text-orange-400" />
        ...
        <div className="flex items-center justify-between pt-4 pb-10">
          <button onClick={() => router.push("/admin/debit-notes-view")} className="px-6 py-2.5 rounded-xl bg-white border border-gray-200 font-bold text-sm hover:bg-gray-50">
            Cancel
          </button>
          <button onClick={handleSubmit} disabled={loading} className={`px-8 py-2.5 rounded-xl text-white font-bold text-sm shadow-lg ${loading ? "bg-gray-300" : "bg-orange-600 hover:bg-orange-700"}`}>
            {loading ? "Processing..." : isEdit ? "Update Debit Note" : "Create Debit Note"}
          </button>
        </div>
      </div>
...
//             <input type="file" multiple accept="image/*,application/pdf" hidden onChange={e => setAttachments([...attachments, ...Array.from(e.target.files)])}  onFocus={(e) => e.target.select()}/>
//           </label>
//         </SectionCard>
