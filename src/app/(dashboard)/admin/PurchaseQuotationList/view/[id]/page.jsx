...
import { 
  ...
  FaWarehouse, FaHistory, FaSpinner, FaClipboardList, FaImage
} from 'react-icons/fa';
...
// ✅ Component that finds variant image from the already-loaded variants array
const ItemImage = ({ item }) => {
  const [err, setErr] = useState(false);
  
  // Find the variant image if this is a variant item
  const getImageUrl = () => {
    // 1. Direct image on the item
    if (item.imageUrl) return item.imageUrl;
    
    // 2. Check if this is a variant (itemCode differs from base itemCode)
    if (item.item && item.itemCode && item.itemCode !== item.item?.itemCode) {
      // Look for variant in the item.item.variants array
      const variants = item.item?.variants || [];
      const matchedVariant = variants.find(v => v.sku === item.itemCode);
      if (matchedVariant && matchedVariant.imageUrl) {
        return matchedVariant.imageUrl;
      }
    }
    
    // 3. Fallback to base item image
  ...
  if (!imageUrl || err) {
    return (
      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-dashed border-gray-200 bg-gray-50 flex items-center justify-center shrink-0">
        <FaImage className="text-gray-300 text-sm sm:text-base" />
      </div>
    );
  }
  ...
  return (
    <img
      ...
      className="w-10 h-10 sm:w-12 sm:h-12 object-cover rounded-xl border border-gray-200 shrink-0"
      onError={() => setErr(true)}
    />
  );
};
  ...
  useEffect(() => {
    ...
    const fetchQuotation = async () => {
      try {
        ...
        const response = await axios.get(`/api/purchase-quotation?id=${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        if (response.data.success) {
          console.log("Fetched quotation:", response.data.data);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <p className="text-gray-400 font-bold uppercase text-xs tracking-widest text-center">Fetching Quotation Details...</p>
      </div>
    );
  }
          ...
          <div className="lg:col-span-8 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 sm:p-6 lg:p-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <DetailField label="Supplier Name" value={quotation.supplierName} />
                <DetailField label="Supplier Code" value={quotation.supplierCode} />
                <DetailField label="Contact Person" value={quotation.contactPerson} />
                <DetailField label="Invoice Type" value={quotation.invoiceType} color="text-indigo-600" />
            ...
            {/* Quoted Items Table */}
            <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
              <SectionHeader icon={FaBoxOpen} title="Quoted Items Manifest" color="emerald" />
                ...
                <table className="min-w-[800px] w-full divide-y divide-gray-100">
                  <thead className="bg-gray-50/50">
                    <tr className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-gray-400">
                      <th className="px-4 sm:px-6 py-3 sm:py-4 text-left">Item</th>
                      <th className="px-3 sm:px-4 py-3 sm:py-4 text-center">Qty</th>
                      <th className="px-3 sm:px-4 py-3 sm:py-4 text-center">Unit Price</th>
                      <th className="px-4 sm:px-6 py-3 sm:py-4 text-right">Total</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {quotation.items?.map((item, idx) => {
                      const isVariant = item.itemCode !== item.item?.itemCode;
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {isVariant && (
                                  <p className="text-[9px] sm:text-[10px] text-purple-600 font-bold mt-1">
                                    ⚡ Variant of: {item.item?.itemName}
                                  </p>
                                )}
                                {item.itemDescription && (
                                  <p className="text-[9px] sm:text-[10px] text-gray-400 italic mt-1">
                                    {item.itemDescription}
                                  </p>
                                )}
                              </div>
                  ...
                  <tfoot className="bg-gray-50 border-t border-gray-200">
                    <tr className="border-b border-gray-100">
                      ...
                      <td className="px-4 sm:px-6 py-3 text-right font-black text-sm sm:text-base">
                        {formatCurrency(quotation.totalBeforeDiscount)}
                       </td>
                     </tr>
                    {quotation.gstTotal > 0 && (
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="px-4 sm:px-6 py-2 text-right text-xs sm:text-sm font-bold text-emerald-600">
                          {formatCurrency(quotation.gstTotal)}
                         </td>
                       </tr>
                    )}
                    {quotation.freight > 0 && (
                    ...
                    {quotation.rounding !== 0 && (
                      <tr className="border-b border-gray-100">
                        ...
                        <td className="px-4 sm:px-6 py-2 text-right text-xs sm:text-sm">
                          {formatCurrency(quotation.rounding)}
                         </td>
                       </tr>
                    )}
          ...
          <div className="lg:col-span-4 space-y-5 sm:space-y-6">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl sm:rounded-[2.5rem] shadow-2xl p-5 sm:p-8 text-white">
              <div className="flex items-center gap-2 mb-6 sm:mb-8 opacity-50">
                <FaCalculator />
                <h3 className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em]">Financial Summary</h3>
              </div>
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-sm text-emerald-400">{formatCurrency(quotation.gstTotal)}</span>
                </div>
                {quotation.freight > 0 && (
                  <div className="flex justify-between items-center text-xs">
            ...
            <div className="bg-white rounded-2xl sm:rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
                <DetailField label="Document Number" value={quotation.documentNumber} />
                ...
                <DetailField label="Valid Until" value={formatDate(quotation.validUntil)} color="text-rose-600" />
              </div>
            </div>
                  ...
                  <div className="space-y-3">
                    {quotation.attachments.map((file, idx) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs sm:text-sm font-black text-gray-900 truncate">{file.fileName}</p>
