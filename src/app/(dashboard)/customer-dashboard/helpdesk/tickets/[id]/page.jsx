...
export default function TicketView() {
  ...
  // ✅ Load user safely (avoid hydration issue)
  useEffect(() => {
    const u = JSON.parse(localStorage.getItem("user") || "{}");
    setUser(u);
  }, []);

  // ✅ Load ticket (stable function)
  ...
  // ✅ Auto scroll bottom on messages update
  useEffect(() => {
      ...
      <div className="space-y-4 bg-gray-100 p-5 rounded-2xl max-h-[60vh] overflow-y-auto">
        {ticket.messages?.length ? (
          ticket.messages.map((m) => (
            <MessageBubble key={m._id} msg={m} meId={user?.id} />
          ))
        ) : (
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-4">
            <button
              disabled={sending}
              onClick={sendReply}
              className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2 rounded-xl hover:bg-blue-700 disabled:opacity-50"
            >
              {sending ? (
