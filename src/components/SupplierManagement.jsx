...
//   const [viewOpen,    setViewOpen]    = useState(false);
//   const [viewSupplier,setViewSupplier]= useState(null);
//   const [localFiles,  setLocalFiles]  = useState([]); // selected files (not yet uploaded)
...
//   // ── Attachment handlers (local preview only) ──
//   const handleFileSelect = (e) => {
//     const files = Array.from(e.target.files);
//     if (!files.length) return;
//     setLocalFiles(prev => [...prev, ...files]);
...
//   // Submit: upload files + save supplier
//   const handleSubmit = async () => {
//     let allE = {};
//     for (let s = 1; s <= 5; s++) { const v = VALIDATORS[s]; if (v) allE = { ...allE, ...v(sd) }; }
//     if (Object.keys(allE).length) { setErrs(allE); toast.error("Please fix errors before submitting"); return; }
...
//     // Prepare supplier data (exclude attachments field – will be merged on backend)
//     const supplierData = {
//       ...sd,
//       glAccount: sd.glAccount?._id || null,
//       attachments: sd.attachments || "", // existing attachments as string
//     };
...
//   // Step content (only step 5 includes attachments)
//   const renderStep = () => {
//     switch (step) {
//       case 1:
//         return (
...
//       case 5:
//         return (
//             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> <Lbl text ="GL Account" /> <input className={fi("")} name="accountNumber" value={sd.glAccount?.name || ""} onChange={handleChange} placeholder="Bank account number (optional)" />  
//             {/* <div className="border-t border-gray-100 pt-4"><Lbl text="GL Account" /><AccountSearch value={sd.glAccount} onSelect={sel => { setSd(p => ({ ...p, glAccount: sel })); clearErr("glAccount"); }} /><Err k="glAccount" /></div> */}
//             <div className="border-t border-gray-100 pt-4"><Lbl text="Currency" /><select className={fi("")} name="currency" value={sd.currency || "INR"} onChange={handleChange}><option value="INR">INR - Indian Rupee</option><option value="USD">USD - US Dollar</option><option value="EUR">EUR - Euro</option><option value="GBP">GBP - British Pound</option><option value="JPY">JPY - Japanese Yen</option><option value="CNY">CNY - Chinese Yuan</option><option value="AUD">AUD - Australian Dollar</option><option value="CAD">CAD - Canadian Dollar</option><option value="CHF">CHF - Swiss Franc</option><option value="SEK">SEK - Swedish Krona</option><option value="NZD">NZD - New Zealand Dollar</option></select></div>
//           </div>
//         );
//       case 5:
//         return (
...
//               <thead><tr className="bg-gray-50 border-b border-gray-100">{["Code","Supplier","Email","Type","Group","Valid","Actions"].map(h => <th key={h} className="px-4 py-3 text-left text-[10.5px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">{h}</th>)}</tr></thead>
//               <tbody>
//                 {loading ? Array(4).fill(0).map((_, i) => <tr key={i} className="border-b border-gray-50">{Array(7).fill(0).map((__, j) => <td key={j} className="px-4 py-3"><div className="h-3.5 rounded bg-gradient-to-r from-gray-100 via-gray-200 to-gray-100 bg-[length:400%_100%] animate-[shimmer_1.4s_infinite]" /></td>)}</tr>) : filtered.length === 0 ? <tr><td colSpan={7} className="text-center py-16"><div className="text-4xl mb-2 opacity-20">🏭</div><p className="text-sm font-medium text-gray-300">No suppliers found</p></td></tr> : filtered.map(s => (
//                   <tr key={s._id} onClick={() => { setViewSupplier(s); setViewOpen(true); }} className="border-b border-gray-50 hover:bg-indigo-50/30 transition-colors cursor-pointer">
//                     <td className="px-4 py-3"><span className="font-mono text-[11px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded">{s.supplierCode}</span></td>
//                     <td className="px-4 py-3"><p className="font-semibold text-gray-900">{s.supplierName}</p><p className="text-xs text-gray-400">{s.supplierCategory}</p></td>
//                     <td className="px-4 py-3 text-gray-500 text-xs">{s.emailId || <span className="text-gray-200">—</span>}</td>
