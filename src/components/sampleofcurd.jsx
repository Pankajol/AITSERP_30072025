...
export default function CustomerManagement() {
  ...
  // Add Cloudinary config (use env var)
  const CLOUDINARY_CLOUD_NAME =
    process.env.CLOUDINARY_CLOUD_NAME || "dz1gfppll";
  const CLOUDINARY_UPLOAD_PRESET =
    process.env.CLOUDINARY_UPLOAD_PRESET || "application";
  const UPLOAD_PRESET = "application";
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      attachments: cd.attachments.map((att) => ({
        name: att.name,
        size: att.size,
        type: att.type,
        url: att.url,
    ...
    try {
      if (cd._id)
        await axios.put(`/api/customers/${cd._id}`, payload, {
          headers: { Authorization: `Bearer ${token}` },
        });
      else
            ...
            <div className="border-t pt-5">
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs text-gray-400">
                  {cd.attachments?.length || 0} file(s)
                </span>
              </div>
              {cd.attachments?.length > 0 && (
                <div className="space-y-2">
                  {cd.attachments.map((att, idx) => (
                    <div
                      key={idx}
            ...
            <div className="md:hidden">
              {filtered.map((c) => (
                <div key={c._id} className="p-4 border-b">
                  ...
                  <span className="text-[10.5px] font-semibold px-2 py-0.5 rounded-full inline-block mt-1 bg-indigo-50 text-indigo-600">
                    {c.customerType}
                  </span>
                </div>
              ))}
        ...
        {viewModalOpen && viewingCustomer && (
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <p>{viewingCustomer.glAccount?.accountName || "—"}</p>
                  </div>
                </div>
                <div>
                  <label className="text-xs uppercase font-bold text-gray-400">
             ...
             <div>
  <label className="text-xs uppercase font-bold text-gray-400">Attachments</label>
  <div className="space-y-2 mt-1">
    {viewingCustomer.attachments?.length ? (
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {viewingCustomer.attachments.map((att, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg border">
              ...
              <a
                ...
                className="text-indigo-600 underline text-sm truncate flex-1"
              >
                {att.name}
              </a>
            </div>
